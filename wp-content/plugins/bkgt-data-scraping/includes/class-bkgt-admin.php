...
class BKGT_Admin {
    ...
    /**
     * Clean up fake/synthetic teams from database
     */
    public function cleanup_teams() {
        if (!wp_verify_nonce($_POST['nonce'], 'bkgt_admin_nonce') ||
            !current_user_can('manage_options')) {
            wp_die(__('Security check failed', 'bkgt-data-scraping'));
        }
        ...
        try {
            ...
            wp_send_json_error(array(
                'message' => __('Cleanup failed: ', 'bkgt-data-scraping') . $e->getMessage()
            ));
        }
    }
}